...
parser.add_argument('--data', type=str, help='Path to the data of length-of-stay task',
                    default='/home/xxx/mimic4processed/length-of-stay/')
...
train_data, normalizer = load_data_utils.los_load_data(dataloader=train_reader, discretizer=discretizer, normalizer=None)
...
mplot.show()
# exit(0)
val_data = load_data_utils.los_load_data(dataloader=val_reader, discretizer=discretizer, normalizer=normalizer)
test_data = load_data_utils.los_load_data(dataloader=test_reader, discretizer=discretizer, normalizer=normalizer)
...
exit(0)
maxlen = int(24 * 7 * 3 / timestep_) # one weak 1 * 7 * 24 / 1
...
train_x = np.array(common_utils.pad_sequences(train_x, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
train_mask = np.array(common_utils.pad_sequences(train_mask, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
train_cur_mask = np.array(common_utils.pad_sequences(train_cur_mask, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
train_cur_mask = cur_mask_(train_cur_mask)
train_dt = np.array(common_utils.pad_sequences(train_dt, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
train_y = np.array(common_utils.pad_sequences(train_y, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
print('after')
print('5 example <len, mask, curmask>')
print([ sum(p) for p in train_mask[:5]])
print(train_mask[:5])
...
val_y = val_data[1]
val_x = np.array(common_utils.pad_sequences(val_x, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
val_mask = np.array(common_utils.pad_sequences(val_mask, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
val_cur_mask = np.array(common_utils.pad_sequences(val_cur_mask, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
val_cur_mask = cur_mask_(val_cur_mask)
val_dt = np.array(common_utils.pad_sequences(val_dt, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
val_y = np.array(common_utils.pad_sequences(val_y, maxlen=maxlen, padding='post', truncating='post'),dtype=float)
...
test_y = test_data[1]
test_x = np.array(common_utils.pad_sequences(test_x, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
test_mask = np.array(common_utils.pad_sequences(test_mask, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
test_cur_mask = np.array(common_utils.pad_sequences(test_cur_mask, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
test_cur_mask = cur_mask_(test_cur_mask)
test_dt = np.array(common_utils.pad_sequences(test_dt, maxlen=maxlen, padding='post', truncating='post'), dtype=float)
test_y = np.array(common_utils.pad_sequences(test_y, maxlen=maxlen, padding='post', truncating='post'),dtype=float)
...
#  mimic4
#  (42553, 168, 76) (42553, 168) (42553, 168) (42553, 168, 10)
#  (7693, 168, 76) (7693, 168) (7693, 168) (7693, 168, 10)
#  (8794, 168, 76) (8794, 168) (8794, 168) (8794, 168, 10)

pickle_file = open('/home/xxx/ehr_data/los/los_mimic4_train_data.pkl', 'wb')
pickle.dump(train_dict, pickle_file)
pickle_file.close()

pickle_file = open('/home/xxx/ehr_data/los/los_mimic4_val_data.pkl', 'wb')
pickle.dump(val_dict, pickle_file)
pickle_file.close()

pickle_file = open('/home/xxx/ehr_data/los/los_mimic4_test_data.pkl', 'wb')
