...
from Predictor import LOS_Predictor
from LossMetrics import EHR_Dataset, EarlyStopping, los_predictor_loss, print_metrics_multiclass
...
def parse_arguments(parser):
    ...
    parser.add_argument('--label_dim', type=int, default=10, help='Dimension of predictor_output_units')
...
if __name__ == '__main__':
    ...
    logging_file = f'/home/xxx/projects/HPformer/out_log/our_los_mimic{args.mimic_dataset:d}_results_log.log'
    ...
    for baseline_name in baseline_names:
        ...
        if args.mimic_dataset == 3:
            train_data_file = '/home/xxx/ehr_data/los/los_mimic3_train_data.pkl'
            val_data_file = '/home/xxx/ehr_data/los/los_mimic3_val_data.pkl'
            test_data_file = '/home/xxx/ehr_data/los/los_mimic3_test_data.pkl'
            saved_model_file = f'/home/xxx/ehr_data/dp/{baseline_name:s}-MIMIC-III'
            os.environ["CUDA_VISIBLE_DEVICES"] = '1'
        else:
            train_data_file = '/home/xxx/ehr_data/los/los_mimic4_train_data.pkl'
            val_data_file = '/home/xxx/ehr_data/los/los_mimic4_val_data.pkl'
            test_data_file = '/home/xxx/ehr_data/los/los_mimic4_test_data.pkl'
        ...
        model = LOS_Predictor(core_model=core_model, label_dim=args.label_dim).to(device)
        ...
        for n_epoch in range(args.epochs):
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                loss = los_predictor_loss(y_true=train_y, y_pred=train_y_hat, mask_=train_mask)
            ...
            with torch.no_grad():
                ...
                for i_batch, batch in enumerate(val_data_loader):
                    ...
                    valid_loss = los_predictor_loss(y_true=val_y, y_pred=val_y_hat, mask_=val_mask)
                    # valid_loss = torch.nn.BCELoss()(input=val_y_hat, target=val_y)
                    cur_val_loss.append(valid_loss.cpu().detach().numpy())
                    val_y = val_y.reshape((-1, 10))
                    val_y_hat = val_y_hat.reshape((-1, 10))
                    for m, t, p in zip(val_mask.cpu().numpy().flatten(), val_y.cpu().numpy(),
                                       val_y_hat.cpu().detach().numpy()):
                        if np.equal(int(m), 1):
                            val_true.append(t)
                            val_pred.append(p)

                val_ret_microf1 = (print_metrics_multiclass(val_true, val_pred))['Kappa']
                early_stopping(val_ret_microf1, n_epoch, model)
        ...
        los_test = EHR_Dataset(test_data_file)
        all_test = los_test.__len__()
        test_data_loader = DataLoader(los_test, batch_size=args.batch_size, shuffle=True)
        ...
        with torch.no_grad():
            ...
            for re_time in tqdm(range(args.reseample_times), desc='Testing'):
                ...
                for i_batch, batch in enumerate(test_data_loader):
                    ...
                    test_loss = los_predictor_loss(y_true=test_y, y_pred=test_y_hat, mask_=test_mask)
                    ...
                    for m, t, p in zip(test_mask.cpu().numpy().flatten(), test_y.cpu().numpy(),
                                       test_y_hat.cpu().detach().numpy()):
                        if np.equal(int(m), 1):
                            test_true.append(t)
                            test_pred.append(p)
                ...
                test_ret = print_metrics_multiclass(test_true, test_pred)
                all_test_time_results.append([test_ret['Kappa'],
                                              test_ret['Micro AUROC'],
                                              test_ret['Macro AUROC']])
            ...
            logging.info(baseline_name + ' Avg'
                         f"\tKappa = {avg_all_test_time_results[0]:.6f}" +
                         f"\tMicro AUROC = {avg_all_test_time_results[1]:.6f}" +
                         f"\tMacro AUROC= {avg_all_test_time_results[2]:.6f}"
                         )
            logging.info(baseline_name + ' Std'
                         f"\tKappa = {std_all_test_time_results[0]:.6f}" +
                         f"\tMicro AUROC = {std_all_test_time_results[1]:.6f}" +
                         f"\tMacro AUROC = {std_all_test_time_results[2]:.6f}"
                         )
            logging.info('')
    ...
    pd.DataFrame(
        ...
        columns=['Model', 'Kappa', 'Micro AUROC', 'Macro AUROC'],
        index=None).to_csv(f'/home/xxx/projects/Lee_ehrits/out_log/our_los_mimic{args.mimic_dataset:d}_results.csv', index=None)