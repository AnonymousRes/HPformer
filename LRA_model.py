...
from fairseq.models.lra.hpformer_lra_encoder import HPformerLRAEncoder
...
class LRAModel(FairseqEncoderModel):
    ...
    def __init__(self, args, encoder, task):
        ...
        #     ])
        #     self.classifier_activation = utils.get_activation_fn(args.classifier_activation_fn) # task 1 2 3 4
        #
        ...
        if args.classifier_layers > 0:
            self.classifier.append(nn.Sequential(torch.nn.Linear(in_features=args.classifier_in_dim, out_features=args.classifier_out_dim, bias=True), self.dropout_module))
            self.classifier.extend([
                nn.Sequential(torch.nn.Linear(in_features=args.classifier_out_dim, out_features=args.classifier_out_dim, bias=True), self.dropout_module)
                for _ in range(args.classifier_layers - 1)
            ])
            self.classifier_activation = utils.get_activation_fn(args.classifier_activation_fn) # task 5

        self.sentence_projection_layer = torch.nn.Linear(in_features=args.classifier_out_dim, out_features=self.sentence_out_dim, bias=False) # task 5
    ...
    def add_args(parser):
        ...
        parser.add_argument('--k-times', type=int, metavar='N',
                            help='expansion multiples of K_{comp}, V_{comp} matrices in HPformer')
        ...
        parser.add_argument('--layer-type', choices=['transformer', 'luna', 'lstm', 'flash', 'mega', 'hpformer'])
    ...
    def forward(self, sample):
        ...
        if not self.use_p:
            if self.layer_type in ['transformer', 'lstm', 'flash', 'mega', 'hpformer']:
                sentence_rep = sentence_rep[1]
                # print('if not self.use_p:', sentence_rep.shape)
            elif self.layer_type == 'luna':
                # print('elif self.layer_type == luna', sentence_rep.shape)
                sentence_rep = sentence_rep[1][0]
        else:
            sentence_rep = sentence_rep[1][1].mean(dim=0)
            # print('sentence_rep = sentence_rep[1][1].mean(dim=0)', sentence_rep.shape)
        if 'net_input1' in sample:
            ...
            # print('if net_input1 in sample:', sentence_rep.shape)
        for layer in self.classifier:
            sentence_rep = self.classifier_activation(layer(sentence_rep))
        # print('for layer in self.classifier:', sentence_rep.shape)
        if self.sentence_projection_layer:
            sentence_logits = self.sentence_projection_layer(sentence_rep)
            # print('if self.sentence_projection_layer:', sentence_logits.shape)
        return {
...
class LRAEncoder(FairseqEncoder):
    ...
    def __init__(self, args, task):
        ...
        elif args.layer_type == 'hpformer':
            self.encoder = HPformerLRAEncoder(
                tie_layer_weights=getattr(args, 'tie_layer_weights', False),
                padding_idx=padding_idx,
                vocab_size=vocab_size,
                num_encoder_layers=args.encoder_layers,
...
def base_architecture(args):
    args.k_times = getattr(args, 'k_times', 1)
...
@register_model_architecture('lra', 'hpformer_lra_listop')
def hpformer_lra_listop(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'hpformer')
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
    args.activation_fn = getattr(args, 'activation_fn', 'silu')
...
@register_model_architecture('lra', 'hpformer_lra_imdb')
def hpformer_lra_imdb(args):
    args.layer_type = getattr(args, 'layer_type', 'hpformer')
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.max_positions = getattr(args, 'max_positions', 4002)
    args.encoder_normalize_before = getattr(args, 'encoder_normalize_before', True)
...
@register_model_architecture('lra', 'hpformer_lra_aan')
def hpformer_lra_aan(args):
    args.layer_type = getattr(args, 'layer_type', 'hpformer')
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.max_positions = getattr(args, 'max_positions', 4002)
...
@register_model_architecture('lra', 'hpformer_lra_cifar10')
def hpformer_lra_cifar10(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'hpformer')
...
@register_model_architecture('lra', 'hpformer_lra_pf32')
def hpformer_lra_pf32(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.encoder_learned_pos = getattr(args, 'encoder_learned_pos', False)
    args.layer_type = getattr(args, 'layer_type', 'hpformer')
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
...
@register_model_architecture('lra', 'hpformer_lra_pf128')
def hpformer_lra_pf128(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'hpformer')
    args.encoder_layers = getattr(args, 'encoder_layers', 4)
    args.activation_fn = getattr(args, 'activation_fn', 'silu')
